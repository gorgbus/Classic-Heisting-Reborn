function HUDMissionBriefing:init(hud, workspace)
	...
	pg_text:move(0, -pg_text:h() * 1.7)
	self._paygrade_panel:move(0, -pg_text:h() * 1.7)
...
function HUDMissionBriefing:set_player_slot(nr, params)
	...
	slot:child("status"):set_left(w + (slot:child("name"):x() + h))
