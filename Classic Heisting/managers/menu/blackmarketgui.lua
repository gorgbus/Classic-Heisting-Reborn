...
local function make_cosmetic_data(data, cosmetic_id, unlocked, quality, bonus, equipped)
    local crafted = managers.blackmarket:get_crafted_category(data.category)[
        data.prev_node_data and data.prev_node_data.slot]
    local my_cd = tweak_data.blackmarket.weapon_skins[cosmetic_id]
    local new_data = {
        name = cosmetic_id,
        name_localized = my_cd and my_cd.name_id and managers.localization:text(my_cd.name_id) or
            managers.localization:text("bm_menu_no_mod"),
    ...
    new_data.akimbo_gui_data = tweak_data.weapon[crafted.weapon_id] and
        tweak_data.weapon[crafted.weapon_id].akimbo_gui_data
    ...
    if new_data.default_blueprint then
        new_data.comparision_data = managers.blackmarket:get_weapon_stats(new_data.category, new_data.slot,
            new_data.default_blueprint)
    end
    ...
    if new_data.unlocked then
        ...
        if managers.dlc:is_content_achievement_locked("weapon_skins", new_data.name) or
            managers.dlc:is_content_achievement_milestone_locked("weapon_skins", new_data.name) then
        ...
        table.insert(new_data.mini_icons, {
            ...
            bottom = 1,
            right = 1,
            texture = lock_icon or my_cd.is_a_unlockable and "guis/textures/pd2/skilltree/padlock" or
                "guis/textures/pd2/lock_dlc",
...
function BlackMarketGuiTabItem:init(main_panel, data, node, size_data, hide_select_rect, scroll_tab_table)
    ...
    for index, data in ipairs(self._data) do
        ...
        local new_slot = new_slot_class:new(self._grid_scroll_panel, data,
            padding_w + (index - 1) % slots[1] * (square_w + padding_w), padding_h + y_index * (square_h + padding_h),
            square_w, square_h)
...
function BlackMarketGuiTabItem:check_grab_scroll_bar(x, y)
    ...
    if self._scroll_bar_panel:child("scroll_up_indicator_arrow"):visible() and
        self._scroll_bar_panel:child("scroll_up_indicator_arrow"):inside(x, y) then
        self._my_node_data.scroll_y_index = math.max(self._my_node_data.scroll_y_index - 1, 1)
        self._pressing_arrow_up = true
        self:set_scroll_y()
        return true
    end
    if self._scroll_bar_panel:child("scroll_down_indicator_arrow"):visible() and
        self._scroll_bar_panel:child("scroll_down_indicator_arrow"):inside(x, y) then
...
function BlackMarketGuiTabItem:set_scroll_indicators()
    ...
    if grid_scroll_panel:h() == 0 then
        Application:error("[BlackMarketGuiTabItem:set_scroll_indicators] Dodging division by zero.", "grid_scroll_panel"
            , inspect(self._data), inspect(self))
        return
    end
...
function BlackMarketGuiSlotItem:init(main_panel, data, x, y, w, h)
    ...
    if data.mid_text and type(data.mid_text) == "table" then
        local text = self._panel:text({
            name = "text",
            text = data.mid_text.no_upper and data.mid_text.noselected_text or
                utf8.to_upper(data.mid_text.noselected_text),
        ...
        for k, icon_data in ipairs(data.mini_icons) do
            ...
            elseif icon_data.stream then
                if DB:has(Idstring("texture"), icon_data.texture) then
                    icon_data.request_index = managers.menu_component:request_texture(icon_data.texture,
                        callback(self, self, "icon_loaded_clbk", icon_data)) or false
                end
            else
    ...
    if data.bitmap_texture then
        if DB:has(Idstring("texture"), data.bitmap_texture) then
            ...
            if data.stream then
                ...
                self._request_index = managers.menu_component:request_texture(self._requested_texture,
                    texture_loaded_clbk)
            else
...
function BlackMarketGuiSlotItem:get_texure_size(debug)
    if self._bitmap then
        ...
        if texture_width == 0 or texture_height == 0 or panel_width == 0 or panel_height == 0 then
            if debug then
                Application:error("BlackMarketGuiSlotItem:get_texure_size(): " .. self._data.bitmap_texture or
                    self._data.bg_texture or "")
                Application:debug("BlackMarketGuiSlotItem:", "texture_width " .. texture_width,
                    "texture_height " .. texture_height, "panel_width " .. panel_width, "panel_height " .. panel_height)
            end
            return 0, 0
        end
...
function BlackMarketGuiSlotItem:select(instant, no_sound)
    ...
    if self._text_in_mid and alive(self._panel:child("text")) then
        ...
        self._panel:child("text"):set_text(self._data.mid_text.no_upper and self._data.mid_text.selected_text or
            utf8.to_upper(self._data.mid_text.selected_text or ""))
...
function BlackMarketGuiSlotItem:deselect(instant)
    ...
    if self._text_in_mid and alive(self._panel:child("text")) then
        ...
        self._panel:child("text"):set_text(self._data.mid_text.no_upper and self._data.mid_text.noselected_text or
            utf8.to_upper(self._data.mid_text.noselected_text or ""))
        if alive(self._lock_bitmap) and self._data.mid_text.is_lock_same_color then
            self._lock_bitmap:set_color(self._panel:child("text"):color())
        end
    end
...
function BlackMarketGuiMaskSlotItem:init(main_panel, data, x, y, w, h)
    ...
    self._mask_name_text:set_text(data.my_part_data.is_good and managers.localization:text(data.my_part_data.text) or
        "NOT SELECTED")
    self._mask_name_text:set_blend_mode(data.my_part_data.is_good and "normal" or "add")
    self._mask_name_text:set_color(data.my_part_data.is_good and tweak_data.screen_colors.text or
    ...
    current_match_with_true = data.my_part_data.is_good and data.my_true_part_data and
        data.my_part_data.id == data.my_true_part_data.id
...
function BlackMarketGui:_setup(is_start_page, component_data)
    ...
    for i, data in ipairs(self._data) do
        ...
        local new_tab = BlackMarketGuiTabItem:new(self._panel, data, self._node, size_data, not self._pages,
            self._tab_scroll_table)
    ...
    if self._tabs[self._selected] then
        ...
        if managers.menu:is_pc_controller() and
            self._tab_scroll_table[#self._tab_scroll_table]:right() > self._tab_scroll_table.panel:w() then
            local prev_page = self._panel:text({
                name = "prev_page",
                y = 0,
                w = 0,
    ...
    if MenuBackdropGUI then
        ...
        if managers.menu:is_pc_controller() then
            ...
            local x, y = managers.gui_data:safe_to_full_16_9(self._panel:child("back_button"):world_right(),
                self._panel:child("back_button"):world_center_y())
        ...
        for i, instance_id in ipairs(new_givens) do
            ...
            elseif inventory_tradable[instance_id] then
                ...
                if ti_td then
                    ...
                    params.rarity_name = managers.localization:text(tweak_data.economy.rarities[ti_td.rarity or "common"
                        ] and tweak_data.economy.rarities[ti_td.rarity or "common"].name_id or "nil")
                    params.quality_name = managers.localization:text(tweak_data.economy.qualities[
                        params.item.quality or "poor"] and
                        tweak_data.economy.qualities[params.item.quality or "poor"].name_id or "nil")
                    params.container = params.item.category == "safes" and {
                        content = tweak_data.economy.safes[params.item.entry] and
                            tweak_data.economy.safes[params.item.entry].content,
                        drill = ti_td.drill,
...
function BlackMarketGui:_update_borders()
    ...
    local weapon_info_height = info_box_panel:h() - (self._button_count > 0 and self._btn_panel:h() + 8 or 0) -
        (self._detection_panel:visible() and self._detection_panel:h() + 8 or 0)
    ...
    self._btn_panel:set_top((
        self._detection_panel:visible() and self._detection_panel:bottom() or self._weapon_info_panel:bottom()) + 8)
...
function BlackMarketGui:get_weapon_ammo_info(weapon_id, extra_ammo, total_ammo_mod)
    ...
    ammo_max_multiplier = ammo_max_multiplier *
        managers.player:upgrade_value(weapon_tweak_data.category, "extra_ammo_multiplier", 1)
    ...
    local ammo_from_mods = math.round(ammo_max *
        (total_ammo_mod and tweak_data.weapon.stats.total_ammo_mod[total_ammo_mod] or 0))
    ammo_max = math.round((
        ammo_max + ammo_from_mods + managers.player:upgrade_value(weapon_id, "clip_amount_increase") * ammo_max_per_clip
        ) * ammo_max_multiplier)
    ...
    ammo_data.skill_in_effect = managers.player:has_category_upgrade("player", "extra_ammo_multiplier") or
        managers.player:has_category_upgrade(weapon_tweak_data.category, "extra_ammo_multiplier") or
...
function BlackMarketGui:_get_skill_stats(name, category, slot, base_stats, mods_stats, silencer, single_mod, auto_mod)
    ...
    local detection_risk = managers.blackmarket:get_suspicion_offset_from_custom_data(custom_data,
    ...
    for _, stat in ipairs(self._stats_shown) do
        if weapon_tweak.stats[stat.stat_name or stat.name] or stat.name == "totalammo" or stat.name == "fire_rate" then
            if stat.name == "magazine" then
                ...
                if not weapon_tweak.upgrade_blocks or not weapon_tweak.upgrade_blocks.weapon or
                    not table.contains(weapon_tweak.upgrade_blocks.weapon, "clip_ammo_increase") then
                    skill_stats[stat.name].value = skill_stats[stat.name].value +
                        managers.player:upgrade_value("weapon", "clip_ammo_increase", 0)
                end
                if not weapon_tweak.upgrade_blocks or not weapon_tweak.upgrade_blocks[weapon_tweak.category] or
                    not table.contains(weapon_tweak.upgrade_blocks[weapon_tweak.category], "clip_ammo_increase") then
                    skill_stats[stat.name].value = skill_stats[stat.name].value +
                        managers.player:upgrade_value(weapon_tweak.category, "clip_ammo_increase", 0)
                end
            ...
            elseif stat.name == "totalammo" then
                ...
                if stat.name == "damage" then
                    multiplier = managers.blackmarket:damage_multiplier(name, weapon_tweak.category, silencer,
                        detection_risk, nil, blueprint)
                    modifier = math.floor(managers.blackmarket:damage_addend(name, weapon_tweak.category, silencer,
                        detection_risk, nil, blueprint) * tweak_data.gui.stats_present_multiplier * multiplier)
                elseif stat.name == "spread" then
                    local fire_mode = single_mod and "single" or auto_mod and "auto" or weapon_tweak.FIRE_MODE or
                        "single"
                    multiplier = managers.blackmarket:accuracy_multiplier(name, weapon_tweak.category, silencer, nil,
                        fire_mode, blueprint)
                elseif stat.name == "recoil" then
                    ...
                    modifier = -managers.blackmarket:recoil_addend(name, weapon_tweak.category, silencer, blueprint) *
                ...
                elseif stat.name == "fire_rate" then
                    multiplier = managers.blackmarket:fire_rate_multiplier(name, weapon_tweak.category, silencer,
                        detection_risk, nil, blueprint)
                end
                if stat.revert then
                    ...
                    if part_data.stats then
                        if stat.name == "magazine" then
                            ...
                            mods_stats[stat.name].value = mods_stats[stat.name].value +
                                (ammo and tweak_data.weapon.stats.extra_ammo[ammo] or 0)
                        elseif stat.name == "totalammo" then
                            local ammo = part_data.stats.total_ammo_mod
                            mods_stats[stat.name].index = mods_stats[stat.name].index + (ammo or 0)
                        else
                            mods_stats[stat.name].index = mods_stats[stat.name].index +
                                (part_data.stats[stat.name] or 0
                                )
                        end
                    end
                end
        ...
        for _, stat in pairs(self._stats_shown) do
            ...
            if mods_stats[stat.name].index and tweak_stats[stat_name] then
                if stat.name == "concealment" then
                    ...
                    index = math.clamp(base_stats[stat.name].index + mods_stats[stat.name].index, 1,
                        #tweak_stats[stat_name])
                end
                mods_stats[stat.name].value = math.round(stat.index and index or
                    tweak_stats[stat_name][index] * tweak_data.gui.stats_present_multiplier)
                local offset = math.round(math.min(tweak_stats[stat_name][1],
                    tweak_stats[stat_name][#tweak_stats[stat_name]]) * tweak_data.gui.stats_present_multiplier)
                if stat.offset then
                    mods_stats[stat.name].value = mods_stats[stat.name].value - offset
                end
                if stat.revert then
                    local max_stat = math.round(math.max(tweak_stats[stat_name][1],
                        tweak_stats[stat_name][#tweak_stats[stat_name]]) * tweak_data.gui.stats_present_multiplier)
                    if stat.revert then
                ...
                if modifier_stats and modifier_stats[stat.name] then
                    ...
                    if stat.revert and not stat.index then
                        ...
                        local offset = math.round(math.min(tweak_stats[stat_name][1],
                            tweak_stats[stat_name][#tweak_stats[stat_name]]))
                        if stat.offset then
                        ...
                        if mod ~= 0 and
                            (
                            modded_value > tweak_stats[stat_name][1] or
                                modded_value < tweak_stats[stat_name][#tweak_stats[stat_name]]) then
...
function BlackMarketGui:_get_base_stats(name)
    ...
    for _, stat in pairs(self._stats_shown) do
        ...
        elseif tweak_stats[stat.name] then
            ...
            base_stats[stat.name].value = math.round(stat.index and index or
                tweak_stats[stat.name][index] * tweak_data.gui.stats_present_multiplier)
            local offset = math.round(math.min(tweak_stats[stat.name][1],
                tweak_stats[stat.name][#tweak_stats[stat.name]
                ]) * tweak_data.gui.stats_present_multiplier)
            if stat.offset then
                base_stats[stat.name].value = base_stats[stat.name].value - offset
            end
            if stat.revert then
                local max_stat = math.round(math.max(tweak_stats[stat.name][1],
                    tweak_stats[stat.name][#tweak_stats[stat.name]]) * tweak_data.gui.stats_present_multiplier)
                if stat.revert then
                    max_stat = max_stat - offset
                end
            ...
            if modifier_stats and modifier_stats[stat.name] then
                ...
                if stat.revert and not stat.index then
                    ...
                    if mod ~= 0 and
                        (
                        modded_value > tweak_stats[stat.name][1] or
                            modded_value < tweak_stats[stat.name][#tweak_stats[stat.name]]) then
...
function BlackMarketGui:_get_melee_weapon_stats(name)
    ...
    for i, stat in ipairs(self._mweapon_stats_shown) do
        ...
        if stat.name == "damage" then
            ...
            local dmg_mul = managers.player:upgrade_value("player",
                "melee_" ..
                tostring(tweak_data.blackmarket.melee_weapons[name].stats.weapon_type) .. "_damage_multiplier", 1)
            local skill_mul = dmg_mul * ((has_non_special and has_special and math.max(non_special, special) or 0) + 1) -
                1
        ...
        elseif stat.name == "damage_effect" then
            ...
            local dmg_mul = managers.player:upgrade_value("player",
                "melee_" ..
                tostring(tweak_data.blackmarket.melee_weapons[name].stats.weapon_type) .. "_damage_multiplier", 1) - 1
    ...
    for i, stat in ipairs(self._mweapon_stats_shown) do
        if not stat.index then
            ...
            if skill_stats[stat.name].min_value and base_stats[stat.name].min_value then
                skill_stats[stat.name].min_value = math.round(base_stats[stat.name].min_value *
                    skill_stats[stat.name].min_value)
            ...
            if skill_stats[stat.name].max_value and base_stats[stat.name].max_value then
                skill_stats[stat.name].max_value = math.round(base_stats[stat.name].max_value *
                    skill_stats[stat.name].max_value)
...
function BlackMarketGui:_get_armor_stats(name)
    ...
    local mods_stats = {}
    local skill_stats = {}
    local detection_risk = managers.blackmarket:get_suspicion_offset_from_custom_data({ armors = name },
        tweak_data.player.SUSPICION_OFFSET_LERP or 0.75)
    detection_risk = math.round(detection_risk * 100)
    ...
    for i, stat in ipairs(self._armor_stats_shown) do
            ...
            skill_stats[stat.name] = {
                value = math.round((
                    base_stats[stat.name].value +
                        managers.player:body_armor_skill_addend(name) * tweak_data.gui.stats_present_multiplier) *
                    managers.player:body_armor_skill_multiplier() - base_stats[stat.name].value)
            }
        elseif stat.name == "concealment" then
            base_stats[stat.name] = {
        ...
        elseif stat.name == "damage_shake" then
            ...
            local skill_value = base / mod / skill - base_value - mod_value +
                managers.player:upgrade_value("player", "damage_shake_addend", 0)
            base_stats[stat.name] = {
        ...
        skill_stats[stat.name].skill_in_effect = skill_stats[stat.name].skill_in_effect or
            skill_stats[stat.name].value ~= 0
    end
    return base_stats, mods_stats, skill_stats
end
...
function BlackMarketGui:_get_stats(name, category, slot)
    ...
    local skill_stats = self:_get_skill_stats(name, category, slot, base_stats, mods_stats, silencer, single_mod,
        auto_mod)
    local clip_ammo, max_ammo, ammo_data = self:get_weapon_ammo_info(name, tweak_data.weapon[name].stats.extra_ammo,
        base_stats.totalammo.index + mods_stats.totalammo.index)
...
function BlackMarketGui:_get_weapon_mod_stats(mod_name, weapon_name, base_stats, mods_stats, equipped_mods)
    ...
    for _, mod in pairs(mod_stats) do
        part_data = nil
        if mod.name then
            part_data = managers.weapon_factory:get_part_data_by_part_id_from_weapon(mod.name, factory_id,
                default_blueprint)
        end
        for _, stat in pairs(self._stats_shown) do
            if part_data and part_data.stats then
                ...
                elseif stat.name == "totalammo" then
                    ...
                    if tweak_stats[stat.name] then
                        ...
                        mod[stat.name] = math.round(stat.index and index or
                            tweak_stats[stat.name][index] * tweak_data.gui.stats_present_multiplier)
                        local offset = math.round(math.min(tweak_stats[stat.name][1],
                            tweak_stats[stat.name][#tweak_stats[stat.name]]) * tweak_data.gui.stats_present_multiplier)
                        if stat.offset then
                        ...
                        if modifier_stats and modifier_stats[stat.name] then
                            ...
                            if stat.revert and not stat.index then
                                ...
                                local new_value = math.round((max_stat - modded_value) *
                                    tweak_data.gui.stats_present_multiplier)
                                if mod_stat ~= 0 and
                                    (
                                    modded_value > tweak_stats[stat.name][1] or
...
function BlackMarketGui:set_stats_titles(...)
    ...
    for i, stat_title in ipairs(stat_title_changes) do
        ...
        local text = stat_title.text or stat_title.text_id and managers.localization:to_upper_text(stat_title.text_id) or
            false
...
function BlackMarketGui:populate_gloves(data)
    ...
    for i, glove_id in ipairs(tweak_data.blackmarket.glove_list) do
        ...
        if Global.blackmarket_manager.gloves[glove_id] and
            (
            not global_value_tweak or not global_value_tweak.hide_unavailable or
                not not managers.dlc:is_global_value_unlocked(tweak.global_value)) then
    ...
    for sort_number, glove_id in ipairs(sort_data) do
        ...
        local achievement_locked = managers.dlc:is_content_achievement_locked("gloves", glove_id) or
            managers.dlc:is_content_achievement_milestone_locked("gloves", glove_id)
    ...
    local mannequin_glove = data.mannequin_glove_id or managers.menu_scene and managers.menu_scene:get_glove_id() or
        "none"
    ...
    for i = 1, max_items do
        ...
        if glove_id then
            ...
            if is_dlc_locked then
                ...
                new_data.dlc_locked = tweak_data.lootdrop.global_values[new_data.global_value] and
                    tweak_data.lootdrop.global_values[new_data.global_value].unlock_id or "bm_menu_dlc_locked"
            else
            ...
            if new_data.unlocked then
                ...
                if dlc_data and dlc_data.app_id and not dlc_data.external and
                    not managers.dlc:is_dlc_unlocked(new_data.global_value) then
...
function BlackMarketGui:populate_armor_skins(data)
    ...
    table.sort(sort_data, function(a, b)
        ...
        return managers.localization:text(ad and ad.name_id or "error") <
            managers.localization:text(bd and bd.name_id or "error")
    end)
    ...
    for i, skin_id in ipairs(sort_data) do
        ...
        local is_locked = tweak_data.lootdrop.global_values[new_data.global_value] and
            tweak_data.lootdrop.global_values[new_data.global_value].dlc and
            not managers.dlc:is_dlc_unlocked(new_data.global_value)
...
function BlackMarketGui:populate_weapon_category(category, data)
    ...
    for i, crafted in pairs(crafted_category) do
        guis_catalog = "guis/"
        local bundle_folder = tweak_data.weapon[crafted.weapon_id] and
            tweak_data.weapon[crafted.weapon_id].texture_bundle_folder
        ...
        new_data.global_value = tweak_data.weapon[new_data.name] and tweak_data.weapon[new_data.name].global_value or
            "normal"
        ...
        local icon_list = managers.menu_component:create_weapon_mod_icon_list(crafted.weapon_id, category,
            crafted.factory_id, i)
        local icon_index = 1
        local new_parts = {}
        ...
        if _G.ch_settings.settings.u24_progress then
            for _, icon in pairs(icon_list) do
                if icon.type == "charm" or icon.type == "weapon_cosmetics" then
                    icon_list[_] = nil
                end
            end
        end
        ...
        if currently_holding then
            ...
            if has_mods and new_data.unlocked and managers.workshop and managers.workshop:enabled() and
                not table.contains(managers.blackmarket:skin_editor():get_excluded_weapons(), new_data.name) then
    ...
    for i = 1, max_items do
        if not data[i] then
            ...
            if can_buy_weapon then
                ...
                new_data.mid_text.selected_text = currently_holding and new_data.mid_text.noselected_text or
                    managers.localization:text("bm_menu_btn_buy_new_weapon")
                new_data.mid_text.selected_color = currently_holding and new_data.mid_text.noselected_color or
                ...
                elseif managers.money:can_afford_buy_weapon_slot() then
                    ...
                    new_data.dlc_locked = new_data.dlc_locked ..
                        "  " .. managers.localization:to_upper_text("bm_menu_cannot_buy_weapon_slot")
...
function BlackMarketGui:populate_armors(data)
    ...
    for i, armor_id in ipairs(sort_data) do
        ...
        guis_catalog = "guis/"
        local bundle_folder = tweak_data.blackmarket.armors[armor_id] and
            tweak_data.blackmarket.armors[armor_id].texture_bundle_folder
        ...
        if i ~= 1 and (game_state_machine and game_state_machine:current_state_name()) ~= "ingame_waiting_for_players"
            and not _G.ch_settings.settings.u24_progress then
...
function BlackMarketGui:show_stats()
    if not self._stats_panel or not self._rweapon_stats_panel or not self._armor_stats_panel or
        not self._mweapon_stats_panel then
        return
    end
    ...
    if tweak_data.weapon[self._slot_data.name] then
        ...
        local equip_base_stats, equip_mods_stats, equip_skill_stats = WeaponDescription._get_stats(equipped_name,
            category, equipped_slot)
        local base_stats, mods_stats, skill_stats = WeaponDescription._get_stats(name, category, slot,
            self._slot_data.default_blueprint)
        ...
        for _, stat in ipairs(self._stats_shown) do
            ...
            if slot == equipped_slot then
                ...
                self._stats_texts[stat.name].mods:set_text(mods_stats[stat.name].value == 0 and "" or
                    (mods_stats[stat.name].value > 0 and "+" or "") ..
                    format_round(mods_stats[stat.name].value, stat.round_value))
                self._stats_texts[stat.name].skill:set_text(skill_stats[stat.name].skill_in_effect and
                    (skill_stats[stat.name].value > 0 and "+" or "") ..
                    format_round(skill_stats[stat.name].value, stat.round_value) or "")
                self._stats_texts[stat.name].total:set_text("")
                ...
                if base < value then
                    self._stats_texts[stat.name].equip:set_color(stat.inverted and
                        tweak_data.screen_colors.stats_negative or tweak_data.screen_colors.stats_positive)
                elseif value < base then
                    self._stats_texts[stat.name].equip:set_color(stat.inverted and
                        tweak_data.screen_colors.stats_positive or tweak_data.screen_colors.stats_negative)
                else
                ...
                elseif tweak_stats[stat.name] then
                    ...
                    local max_stat = math.max(tweak_stats[stat.name][1], tweak_stats[stat.name][#tweak_stats[stat.name]])
                        * tweak_data.gui.stats_present_multiplier * (modifier_stats and modifier_stats[stat.name] or 1)
                    if stat.offset then
                        local offset = math.min(tweak_stats[stat.name][1],
                            tweak_stats[stat.name][#tweak_stats[stat.name]]) * tweak_data.gui.stats_present_multiplier *
                ...
                local equip = math.max(equip_base_stats[stat.name].value + equip_mods_stats[stat.name].value +
                    equip_skill_stats[stat.name].value, 0)
                ...
                if equip < value then
                    self._stats_texts[stat.name].total:set_color(stat.inverted and
                        tweak_data.screen_colors.stats_negative or tweak_data.screen_colors.stats_positive)
    ...
    elseif tweak_data.blackmarket.armors[self._slot_data.name] then
        ...
        for _, stat in ipairs(self._armor_stats_shown) do
            self._armor_stats_texts[stat.name].name:set_text(utf8.to_upper(managers.localization:text("bm_menu_" ..
                stat.name)))
            value = math.max(base_stats[stat.name].value + mods_stats[stat.name].value + skill_stats[stat.name].value, 0)
            if self._slot_data.name == equipped_slot then
                local base = base_stats[stat.name].value
                ...
                self._armor_stats_texts[stat.name].skill:set_text((0 < skill_stats[stat.name].value and "+" or "") ..
                    skill_stats[stat.name].value or "")
                self._armor_stats_texts[stat.name].total:set_text("")
    ...
    elseif tweak_data.blackmarket.melee_weapons[self._slot_data.name] then
        ...
        for _, stat in ipairs(self._mweapon_stats_shown) do
            self._mweapon_stats_texts[stat.name].name:set_text(utf8.to_upper(managers.localization:text("bm_menu_" ..
                stat.name)))
            if stat.range then
                value_min = math.max(base_stats[stat.name].min_value + mods_stats[stat.name].min_value +
                    skill_stats[stat.name].min_value, 0)
                value_max = math.max(base_stats[stat.name].max_value + mods_stats[stat.name].max_value +
                    skill_stats[stat.name].max_value, 0)
            end
            value = math.max(base_stats[stat.name].value + mods_stats[stat.name].value + skill_stats[stat.name].value, 0)
            if self._slot_data.name == equipped_item then
                ...
                local skill_text = skill_stats[stat.name].value and
                    string.format(format_string, skill_stats[stat.name].value)
                ...
                self._mweapon_stats_texts[stat.name].skill:set_text((0 < skill_stats[stat.name].value and "+" or "") ..
                    skill_text or "")
                self._mweapon_stats_texts[stat.name].total:set_text("")
                ...
                if stat.range then
                    equip_min = math.max(equip_base_stats[stat.name].min_value + equip_mods_stats[stat.name].min_value +
                        equip_skill_stats[stat.name].min_value, 0)
                    equip_max = math.max(equip_base_stats[stat.name].max_value + equip_mods_stats[stat.name].max_value +
                        equip_skill_stats[stat.name].max_value, 0)
                end
                equip = math.max(equip_base_stats[stat.name].value + equip_mods_stats[stat.name].value +
                    equip_skill_stats[stat.name].value, 0)
                local format_string = "%0." .. tostring(stat.num_decimals or 0) .. "f"
                ...
                for _, color_range in ipairs(color_ranges) do
                    self._mweapon_stats_texts[stat.name].total:set_range_color(color_range.start, color_range.stop,
                        color_range.color)
                end
            end
        end
        ...
        local unaltered_total_base_stats, unaltered_total_mods_stats, unaltered_total_skill_stats = WeaponDescription._get_stats(name
            , category, slot, blueprint)

        managers.weapon_factory:change_part_blueprint_only(weapon.factory_id, self._slot_data.name, blueprint, false)

        local total_base_stats, total_mods_stats, total_skill_stats = WeaponDescription._get_stats(name, category, slot,
            blueprint)
        ...
        for _, stat in ipairs(self._stats_shown) do
            ...
            total_value = math.max(total_base_stats[stat.name].value + total_mods_stats[stat.name].value +
                total_skill_stats[stat.name].value, 0)
            unaltered_total_value = math.max(unaltered_total_base_stats[stat.name].value +
                unaltered_total_mods_stats[stat.name].value + unaltered_total_skill_stats[stat.name].value, 0)
            stat_changed = tweak_parts and tweak_parts.stats and tweak_parts.stats[stat.stat_name or stat.name] and
                value ~= 0
            ...
            if not hide_equip then
                self._stats_texts[stat.name].total:set_text(value == 0 and "" or
                    (value > 0 and "+" or "") .. format_round(value, stat.round_value))
            end
            if remove_stats[stat.name] and remove_stats[stat.name] ~= 0 then
                local stat_str = remove_stats[stat.name] == 0 and "" or
                    (remove_stats[stat.name] > 0 and "+" or "") ..
                    format_round(remove_stats[stat.name], stat.round_value)
...
function BlackMarketGui:_stop_rename_item()
    if self._renaming_item then
        managers.blackmarket:set_crafted_custom_name(self._renaming_item.category, self._renaming_item.slot,
            self._renaming_item.custom_name)
...
function BlackMarketGui:update_info_text()
    ...
    local suspicion, max_reached, min_reached = managers.blackmarket:get_suspicion_offset_of_local(tweak_data.player.SUSPICION_OFFSET_LERP
        or 0.75)
    self:_set_detection(suspicion, max_reached, min_reached)
    self:_set_rename_info_text(nil)
    local is_renaming_this = self._renaming_item and not self._data.is_loadout and
        self._renaming_item.category == slot_data.category and self._renaming_item.slot == slot_data.slot
        ...
        if not slot_data.empty_slot then
            ...
            if price > 0 then
                updated_texts[2].text = "##" ..
                    managers.localization:to_upper_text(slot_data.not_moddable and "st_menu_cost" or "st_menu_value") ..
                    " " .. managers.experience:cash_string(price) .. "##"
                table.insert(resource_color,
                    slot_data.can_afford and tweak_data.screen_colors.text or tweak_data.screen_colors.important_1)
            end
            ...
            if not slot_data.unlocked then
                ...
                local dlc_based = tweak_data.lootdrop.global_values[slot_data.global_value] and
                    tweak_data.lootdrop.global_values[slot_data.global_value].dlc and
                    not tweak_data.dlc[slot_data.global_value].free and not managers.dlc:has_dlc(slot_data.global_value)
            ...
            if slot_data.not_moddable then
                ...
                local movement_penalty = weapon_tweak and
                    tweak_data.upgrades.weapon_movement_penalty[weapon_tweak.category] or 1
                if movement_penalty < 1 then
                    local penalty_as_string = string.format("%d%%", math.round((1 - movement_penalty) * 100))
                    updated_texts[5].text = updated_texts[5].text ..
                        managers.localization:to_upper_text("bm_menu_weapon_movement_penalty_info",
        ...
        if slot_data.is_a_color_skin then
            if slot_data.equipped then
                ...
                if not slot_data.unlocked then
                    ...
                    if not unlocked then
                        updated_texts[5].text = managers.localization:text(gvalue_tweak and gvalue_tweak.unlock_id or
                            "bm_menu_dlc_locked")
                    elseif not have_color then
                        ...
                        if achievement and managers.achievment:get_info(achievement) then
                            ...
                            updated_texts[5].text = managers.localization:text(achievement_visual and
                                achievement_visual.desc_id or "achievement_" .. tostring(achievement) .. "_desc" or
                                "bm_menu_dlc_locked")
                        else
                ...
                local color_index_string = managers.localization:to_upper_text("bm_menu_weapon_color_index", {
                    variation = managers.localization:text(tweak_data.blackmarket:get_weapon_color_index_string(slot_data
                        .cosmetic_color_index))
                })
                ...
                table.insert(updated_texts[4].resource_color,
                    tweak_data.economy.qualities[cosmetic_quality].color or tweak_data.screen_colors.text)
            else
                updated_texts[4].text = updated_texts[4].text ..
                    managers.localization:text("bm_menu_customizable_weapon_color_desc")
            end
        else
            if not slot_data.unlocked then
            ...
            if cosmetic_rarity then
                updated_texts[4].text = updated_texts[4].text ..
                    managers.localization:to_upper_text("bm_menu_steam_item_rarity", {
                        rarity = managers.localization:text(tweak_data.economy.rarities[cosmetic_rarity].name_id)
                    })

                table.insert(updated_texts[4].resource_color,
                    tweak_data.economy.rarities[cosmetic_rarity].color or tweak_data.screen_colors.text)
            end

            if cosmetic_quality then
                updated_texts[4].text = updated_texts[4].text ..
                    (cosmetic_rarity and "\n" or "") ..
                    managers.localization:to_upper_text("bm_menu_steam_item_quality", {
                        quality = managers.localization:text(tweak_data.economy.qualities[cosmetic_quality].name_id)
                    })
            ...
            if cosmetic_bonus then
                local bonus = tweak_data.blackmarket.weapon_skins[slot_data.cosmetic_id] and
                    tweak_data.blackmarket.weapon_skins[slot_data.cosmetic_id].bonus

                if bonus then
                    local bonus_tweak = tweak_data.economy.bonuses[bonus]
                    local bonus_value = bonus_tweak.exp_multiplier and bonus_tweak.exp_multiplier * 100 - 100 .. "%" or
                        bonus_tweak.money_multiplier and bonus_tweak.money_multiplier * 100 - 100 .. "%"
                    updated_texts[4].text = updated_texts[4].text ..
                        ((cosmetic_quality or cosmetic_rarity) and "\n" or "") ..
    ...
    elseif identifier == self.identifiers.melee_weapon then
        ...
        if not slot_data.unlocked then
            local skill_based = slot_data.skill_based
            local level_based = slot_data.level and 0 < slot_data.level
            local dlc_based = not slot_data.dlc_based and tweak_data.lootdrop.global_values[slot_data.global_value] and
                tweak_data.lootdrop.global_values[slot_data.global_value].dlc and
                not tweak_data.dlc[slot_data.global_value].free and not managers.dlc:has_dlc(slot_data.global_value)
            local skill_text_id = skill_based and (slot_data.skill_name or "bm_menu_skilltree_locked") or false
            local level_text_id = level_based and "bm_menu_level_req" or false
    ...
    elseif identifier == self.identifiers.armor then
        ...
        if not self._slot_data.unlocked then
            updated_texts[3].text = utf8.to_upper(managers.localization:text(slot_data.level == 0 and
                (slot_data.skill_name or "bm_menu_skilltree_locked") or "bm_menu_level_req", {
                level = slot_data.level,
    ...
    elseif identifier == self.identifiers.armor_skins then
        ...
        if self._slot_data.cosmetic_rarity then
            local rarity_color = tweak_data.economy.rarities[self._slot_data.cosmetic_rarity].color or
    ...
    elseif identifier == self.identifiers.mask then
        ...
        if not slot_data.empty_slot then
            ...
            if price > 0 and slot_data.slot ~= 1 then
                updated_texts[2].text = "##" ..
                    managers.localization:to_upper_text("st_menu_value") ..
                    " " .. managers.experience:cash_string(price) .. "##" .. "   "
                table.insert(resource_colors,
                    slot_data.can_afford ~= false and tweak_data.screen_colors.text or
                    tweak_data.screen_colors.important_1)
            end
            ...
            elseif mask_id and achievement_tracker[mask_id] and
                (type(slot_data.unlocked) ~= "number" and not slot_data.unlocked or slot_data.unlocked == 0) then
                ...
                if stat and max_progress > 0 then
                    ...
                    if progress_left > 0 then
                        ...
                        updated_texts[3].text = "##" ..
                            managers.localization:text(achievement_data.text_id, { progress = progress }) .. "##"
            ...
            if mask_id then
                ...
                updated_texts[4].text = desc_id and managers.localization:text(desc_id) or
                    Application:production_build() and
                    "Add ##desc_id## to ##" .. mask_id .. "## in tweak_data.blackmarket.masks" or ""
                if managers.dlc:is_mask_achievement_locked(mask_id) and
                    (
                    not tweak_data.blackmarket.masks[mask_id].pcs or
                        not (0 < #tweak_data.blackmarket.masks[mask_id].pcs
                        )) then
                    updated_texts[4].text = updated_texts[4].text ..
                        managers.localization:text("bm_msk_achievement_postfix")
                end
                if slot_data.global_value and slot_data.global_value ~= "normal" then
                    updated_texts[4].text = updated_texts[4].text .. [[
...
##]]                 ..
    ...
    elseif identifier == self.identifiers.weapon_mod then
        local price = slot_data.price or
            managers.money:get_weapon_modify_price(prev_data.name, slot_data.name, slot_data.global_value)
        updated_texts[1].text = slot_data.name_localized
        local resource_colors = {}

        if price > 0 then
            updated_texts[2].text = "##" ..
                managers.localization:to_upper_text("st_menu_cost") ..
                " " .. managers.experience:cash_string(price) .. "##"
        ...
        elseif slot_data.free_of_charge then
            updated_texts[2].text = updated_texts[2].text ..
                (
                unlocked > 0 and managers.localization:to_upper_text("bm_menu_item_unlocked") or
                    managers.localization:to_upper_text("bm_menu_item_locked"))
        else
            updated_texts[2].text = updated_texts[2].text ..
                "##" .. managers.localization:to_upper_text("bm_menu_item_amount", {
            ...
            table.insert(resource_colors,
                math.abs(unlocked) > 0 and tweak_data.screen_colors.text or tweak_data.screen_colors.important_1)
        end
                ...
                if achievement_visual then
                    ...
                    if achievement_visual.progress then
                        updated_texts[3].text = updated_texts[3].text ..
                            " (" ..
                            tostring(achievement_visual.progress.get()) ..
                            "/" .. tostring(achievement_visual.progress.max) .. ")"
                    end
        ...
        local is_underbarrel_ammo = part_data and part_data.type == "underbarrel_ammo" or
            perks and table.contains(perks, "underbarrel_ammo")
        ...
        if is_gadget or is_ammo or is_bayonet or is_bipod or is_underbarrel_ammo or has_desc then
            ...
            updated_texts[4].text = managers.weapon_factory:get_part_desc_by_part_id_from_weapon(part_id,
                crafted.factory_id, crafted.blueprint)
        end
            ...
            for category, amount in pairs(forbids) do
                if droppable_mods[category] then
                    ...
                    for _, part_name in ipairs(weapon_data.uses_parts) do
                        ...
                        if part_data and not part_data.unatainable and part_data.type == category and
                            not table.contains(weapon_data.default_blueprint, part_name) then
    ...
    elseif identifier == self.identifiers.mask_mod then
        ...
        updated_texts[2].text = updated_texts[2].text ..
            managers.localization:to_upper_text("bm_menu_masks") .. ": " .. self._data.topic_params.mask_name
        ...
        if mask_mod_info[1].overwritten then
            updated_texts[2].text = updated_texts[2].text ..
                material_text ..
                ": " .. "##" .. managers.localization:to_upper_text("menu_bm_overwritten") .. "##" .. "\n"

            table.insert(resource_color, tweak_data.screen_colors.risk)
        elseif mask_mod_info[1].is_good then
            updated_texts[2].text = updated_texts[2].text ..
                material_text .. ": " .. managers.localization:text(mask_mod_info[1].text)

            if mask_mod_info[1].price and mask_mod_info[1].price > 0 then
                updated_texts[2].text = updated_texts[2].text ..
                    " " .. managers.experience:cash_string(mask_mod_info[1].price)
            end
            ...
            updated_texts[2].text = updated_texts[2].text ..
                material_text ..
                ": " .. "##" .. managers.localization:to_upper_text("menu_bm_not_selected") .. "##" .. "\n"
        ...
        elseif mask_mod_info[2].is_good then
            updated_texts[2].text = updated_texts[2].text ..
                pattern_text .. ": " .. managers.localization:text(mask_mod_info[2].text)

            if mask_mod_info[2].price and mask_mod_info[2].price > 0 then
                updated_texts[2].text = updated_texts[2].text ..
                    " " .. managers.experience:cash_string(mask_mod_info[2].price)
            end
        ...
        local should_show_one_color = mask_mod_info[4].is_same or
            mask_mod_info[3].overwritten and mask_mod_info[4].overwritten
        ...
        if should_show_one_color then
            ...
            elseif mask_mod_info[3].is_good then
                updated_texts[2].text = updated_texts[2].text ..
                    colors_text .. ": " .. managers.localization:text(mask_mod_info[3].text)

                if mask_mod_info[3].price and mask_mod_info[3].price > 0 then
                    updated_texts[2].text = updated_texts[2].text ..
                        " " .. managers.experience:cash_string(mask_mod_info[3].price)
                end
            ...
            elseif mask_mod_info[3].is_good then
                updated_texts[2].text = updated_texts[2].text ..
                    color_a_text .. ": " .. managers.localization:text(mask_mod_info[3].text)
            ...
            elseif mask_mod_info[4].is_good then
                updated_texts[2].text = updated_texts[2].text ..
                    color_b_text .. ": " .. managers.localization:text(mask_mod_info[4].text)

                if mask_mod_info[4].price and mask_mod_info[4].price > 0 then
                    updated_texts[2].text = updated_texts[2].text ..
                        " " .. managers.experience:cash_string(mask_mod_info[4].price)
                end
        ...
        if slot_data.global_value then
            ...
            local mod_price = managers.money:get_mask_part_price_modified(slot_data.category, slot_data.name,
                slot_data.global_value, mask.mask_id) or 0
        ...
        if price and price > 0 then
            updated_texts[2].text = updated_texts[2].text ..
                managers.localization:to_upper_text("menu_bm_total_cost", {
                    cost = (not can_afford and "##" or "") ..
                        managers.experience:cash_string(price) .. (not can_afford and "##" or "")
                })
    ...
    elseif identifier == self.identifiers.player_style then
        ...
        if not slot_data.unlocked then
            updated_texts[2].text = "##" .. managers.localization:to_upper_text("bm_menu_item_locked") .. "##"
            updated_texts[2].resource_color = tweak_data.screen_colors.important_1
            updated_texts[3].text = slot_data.dlc_locked and managers.localization:to_upper_text(slot_data.dlc_locked) or
                managers.localization:to_upper_text("bm_menu_dlc_locked")
        end
    ...
    elseif identifier == self.identifiers.deployable then
        updated_texts[1].text = slot_data.name_localized
        if not self._slot_data.unlocked then
            updated_texts[3].text = managers.localization:to_upper_text(slot_data.level == 0 and
                (slot_data.skill_name or "bm_menu_skilltree_locked") or "bm_menu_level_req", {
                level = slot_data.level,
                SKILL = slot_data.name
            })
    ...
    if is_renaming_this and self._rename_info_text then
        local text = self._renaming_item.custom_name ~= "" and self._renaming_item.custom_name or
            "##" .. tostring(slot_data.raw_name_localized) .. "##"
...
function BlackMarketGui:set_info_text(id, new_string, resource_color)
    ...
    if resource_color then
        ...
        if #start_ci ~= #end_ci then
            Application:error("BlackMarketGui: Missing ##'s in :set_info_text() string!", id, new_string, #start_ci,
                #end_ci)
        else
            for i = 1, #start_ci do
                info_text:set_range_color(start_ci[i], end_ci[i],
                    type(resource_color) == "table" and resource_color[i] or resource_color)
            end
        end
    end
end
...
function BlackMarketGui:mouse_moved(o, x, y)
    ...
    if self._tabs[self._selected] then
        ...
        if tab_used then
            ...
            self._select_rect:set_visible(y > self._tabs[self._selected]._grid_panel:top() and
                y < self._tabs[self._selected]._grid_panel:bottom())
            used = tab_used
            pointer = tab_pointer
        end
    end
...
function BlackMarketGui:mouse_pressed(button, x, y)
    ...
    elseif self._tabs[self._selected] and scroll_button_pressed and
        self._tabs[self._selected]:mouse_pressed(button, x, y) then
        local x, y = self._tabs[self._selected]:selected_slot_center()
        self._select_rect:set_world_center(x, y)
        self._select_rect:stop()
        self._select_rect_box:set_color(Color.white)
    ...
    if self._tab_scroll_panel:inside(x, y) and self._tabs[self._highlighted] and
        self._tabs[self._highlighted]:inside(x, y) ~= 1 then
...
function BlackMarketGui:set_selected_tab(tab, no_sound)
    ...
    if self._selected_slot then
        ...
        visibility_visible = (
            slot_category == "primaries" or slot_category == "secondaries" or slot_category == "armors" or
                slot_category == "melee_weapons") and not self._data.buying_weapon
    end
...
function BlackMarketGui:press_pc_button(button)
    ...
    if btn and btn._data and btn._data.callback and
        (not self._button_press_delay or self._button_press_delay < TimerManager:main():time()) then
        managers.menu_component:post_event("menu_enter")
...
function BlackMarketGui:populate_characters(data)
    local new_data = {}
    local max_items = math.ceil(CriminalsManager.get_num_characters() / (data.override_slots[1] or 3)) *
        (data.override_slots[1] or 3)
    ...
    for i = 1, char_num do
        ...
        guis_catalog = "guis/"
        local character_table = tweak_data.blackmarket.characters[character] or
            tweak_data.blackmarket.characters.locked[character_name]
        ...
        new_data.slot = i
        new_data.unlocked = not character_table or not character_table.dlc or
            managers.dlc:is_dlc_unlocked(character_table.dlc)
        ...
        if character_table and character_table.locks then
            ...
            if achievement and managers.achievment:get_info(achievement) and
                not managers.achievment:get_info(achievement).awarded then
            ...
            elseif dlc and not managers.dlc:is_dlc_unlocked(dlc) then
                new_data.dlc_locked = tweak_data.lootdrop.global_values[dlc] and
                    tweak_data.lootdrop.global_values[dlc].unlock_id or "bm_menu_dlc_locked"
            end
        else
            new_data.dlc_locked = character_table and character_table.dlc and
                tweak_data.lootdrop.global_values[character_table.dlc] and
                tweak_data.lootdrop.global_values[character_table.dlc].unlock_id or "bm_menu_dlc_locked"
        end
        if not new_data.equipped and new_data.unlocked then
    ...
    for i, grenades_data in ipairs(sort_data) do
        ...
        if m_tweak_data and m_tweak_data.locks then
            ...
            elseif dlc and not managers.dlc:is_dlc_unlocked(dlc) then
                ...
                new_data.dlc_locked = tweak_data.lootdrop.global_values[new_data.global_value].unlock_id or
                    "bm_menu_dlc_locked"
            end
        else
            new_data.lock_texture = self:get_lock_icon(new_data)
...
function BlackMarketGui:populate_choose_mask_mod(data)
    ...
    for type, mods in pairs(data.on_create_data) do
        guis_catalog = "guis/"
        local bundle_folder = tweak_data.blackmarket[data.category][mods.id] and
            tweak_data.blackmarket[data.category][mods.id].texture_bundle_folder
        ...
        if new_data.unlocked and type_func(new_data.unlocked) == "number" and
        ...
        if data.category == "mask_colors" then
            ...
            table.insert(new_data.extra_bitmaps,
                "guis/dlcs/mcu/textures/pd2/blackmarket/icons/mask_color/mask_color_icon")
        ...
        elseif data.category == "textures" then
            ...
            new_data.bitmap_texture = guis_catalog ..
                "textures/pd2/blackmarket/icons/" .. tostring(data.category) .. "/" .. new_data.name
        end
...
function BlackMarketGui:populate_masks(data)
    ...
    local mini_icon_helper = math.round((self._panel:h() - (tweak_data.menu.pd2_medium_font_size + 10) - 60) * GRID_H_MUL
        / 3) - 16
    local max_items = data.override_slots and data.override_slots[1] * data.override_slots[2] or 9
    ...
    for i, crafted in pairs(crafted_category) do
        ...
        guis_catalog = "guis/"
        local bundle_folder = tweak_data.blackmarket.masks[guis_mask_id] and
            tweak_data.blackmarket.masks[guis_mask_id].texture_bundle_folder
        if bundle_folder then
            guis_catalog = guis_catalog .. "dlcs/" .. tostring(bundle_folder) .. "/"
        end
        new_data = {}
        ...
        if not is_locked then
            ...
            local default_blueprint = tweak_data.blackmarket.masks[crafted.mask_id] and
                tweak_data.blackmarket.masks[crafted.mask_id].default_blueprint or {}
            for type, part in pairs(crafted.blueprint) do
                if default_blueprint[type] ~= part.id and default_blueprint[name_converter[type]] ~= part.id and
                    tweak_data.lootdrop.global_values[part.global_value] and
                    tweak_data.lootdrop.global_values[part.global_value].dlc and
                    not managers.dlc:is_dlc_unlocked(part.global_value) then
        ...
        if is_locked then
            ...
            elseif 0 < table.size(locked_parts) then
                ...
                if gv then
                    new_data.lock_texture = self:get_lock_icon(new_data, "guis/textures/pd2/lock_incompatible")
                    new_data.dlc_locked = tweak_data.lootdrop.global_values[gv] and
                        tweak_data.lootdrop.global_values[gv].unlock_id or "bm_menu_dlc_locked"
                end
            end
        end
        if currently_holding then
        ...
        if crafted.modded then
            ...
            if pattern == "solidfirst" or pattern == "solidsecond" then
                ...
                guis_catalog = "guis/"
                local bundle_folder = tweak_data.blackmarket.materials[material_id] and
                    tweak_data.blackmarket.materials[material_id].texture_bundle_folder
                if bundle_folder then
                    guis_catalog = guis_catalog .. "dlcs/" .. tostring(bundle_folder) .. "/"
                end
                local right = -3
        ...
        elseif i ~= 1 and managers.blackmarket:can_modify_mask(i) and
            managers.blackmarket:got_new_drop("normal", "mask_mods", crafted.mask_id) then
            new_data.mini_icons = new_data.mini_icons or {}
            table.insert(new_data.mini_icons, {
                name = "new_drop",
                texture = "guis/textures/pd2/blackmarket/inv_newdrop",
        ...
        if not data[i] then
            ...
            if can_buy_masks then
                ...
                elseif managers.money:can_afford_buy_mask_slot() then
                    ...
                    new_data.mid_text.selected_color = tweak_data.screen_colors.important_1
                    new_data.dlc_locked = new_data.dlc_locked ..
                        "  " .. managers.localization:to_upper_text("bm_menu_cannot_buy_mask_slot")
                    new_data.mid_text.lock_noselected_color = tweak_data.screen_colors.important_1
                    new_data.cannot_buy = true
                end
            end
...
function BlackMarketGui:populate_mods(data)
    ...
    local global_values = managers.blackmarket:get_crafted_category(data.prev_node_data.category)[
        data.prev_node_data.slot].global_values or {}
    local gvs = {}
    local mod_t = {}
    ...
    for index, mod_t in ipairs(data.on_create_data) do
        ...
        guis_catalog = "guis/"
        local bundle_folder = tweak_data.blackmarket.weapon_mods[mod_name] and
            tweak_data.blackmarket.weapon_mods[mod_name].texture_bundle_folder
        if bundle_folder then
            guis_catalog = guis_catalog .. "dlcs/" .. tostring(bundle_folder) .. "/"
        end
        new_data = {}
        ...
        new_data.name_localized = managers.weapon_factory:get_part_name_by_part_id(mod_name) or
            managers.localization:text("bm_menu_no_mod")
        ...
        new_data.unlocked = mod_default or
            managers.blackmarket:get_item_amount(new_data.global_value, "weapon_mods", new_data.name, true)
        new_data.equipped = false
        new_data.stream = true
        new_data.default_mod = default_mod
        new_data.is_internal = tweak_data.weapon.factory:is_part_internal(new_data.name)
        new_data.free_of_charge = tweak_data.blackmarket.weapon_mods[mod_name] and
            tweak_data.blackmarket.weapon_mods[mod_name].is_a_unlockable
        ...
        new_data.can_afford = managers.money:can_afford_weapon_modification(weapon_id, new_data.name,
            new_data.global_value)
        ...
        if mod_name then
            ...
            if forbid then
                ...
                new_data.conflict = managers.localization:text("bm_menu_" ..
                    tostring(tweak_data.weapon.factory.parts[forbid] and tweak_data.weapon.factory.parts[forbid].type or
                        forbid))
            end
            local weapon = managers.blackmarket:get_crafted_category_slot(data.prev_node_data.category,
                data.prev_node_data.slot) or {}
            local gadget
            ...
            local is_auto = weapon and tweak_data.weapon[weapon.weapon_id] and
                tweak_data.weapon[weapon.weapon_id].FIRE_MODE == "auto"
            if mod_type == "gadget" then
                gadget = sub_type
            end
            local silencer = sub_type == "silencer" and true
            local texture = managers.menu_component:get_texture_from_mod_type(mod_type, sub_type, gadget, silencer,
                is_auto)
            ...
            if not new_data.conflict and new_data.unlocked and not is_gadget and not new_data.dlc_locked then
                new_data.comparision_data = managers.blackmarket:get_weapon_stats_with_mod(new_data.category,
                    new_data.slot, mod_name)
            end
    ...
    local weapon_blueprint = managers.blackmarket:get_weapon_blueprint(data.prev_node_data.category,
        data.prev_node_data.slot) or {}
    local equipped
    for i, mod in ipairs(data) do
        for _, weapon_mod in ipairs(weapon_blueprint) do
            if mod.name == weapon_mod and
                (not global_values[weapon_mod] or global_values[weapon_mod] == data[i].global_value) then
                equipped = i
            else
            end
        end
    ...
    if equipped then
        ...
        if not data[equipped].conflict then
            if data[equipped].default_mod then
                data[equipped].comparision_data = managers.blackmarket:get_weapon_stats_with_mod(data[equipped].category
                    , data[equipped].slot, data[equipped].default_mod)
            else
                data[equipped].comparision_data = managers.blackmarket:get_weapon_stats_without_mod(data[equipped].category
                    , data[equipped].slot, data[equipped].name)
            end
        end
    end
end
...
function BlackMarketGui:populate_buy_weapon(data)
    ...
    for i = 1, #data.on_create_data do
        ...
        guis_catalog = "guis/"
        local bundle_folder = tweak_data.weapon[data.on_create_data[i].weapon_id] and
            tweak_data.weapon[data.on_create_data[i].weapon_id].texture_bundle_folder
        if bundle_folder then
            guis_catalog = guis_catalog .. "dlcs/" .. tostring(bundle_folder) .. "/"
        end
        new_data = {}
        ...
        elseif Global.dlc_manager.all_dlc_data[new_data.global_value] and
            Global.dlc_manager.all_dlc_data[new_data.global_value].app_id and
            not Global.dlc_manager.all_dlc_data[new_data.global_value].external and
            not managers.dlc:is_dlc_unlocked(new_data.global_value) then
            table.insert(new_data, "bw_buy_dlc")
        end
                ...
                if item_data then
                    for item_id, item_amount in pairs(type_table) do
                        if not item_data[item_id] then
                            print("BlackMarketGui:_cleanup_blackmarket: Missing '" ..
                                item_id .. "' in BlackMarketTweakData. Removing it from stash!")
...
function BlackMarketGui:populate_player_styles(data)
    ...
    for i, player_style in ipairs(tweak_data.blackmarket.player_style_list) do
        ...
        if Global.blackmarket_manager.player_styles[player_style] and
            (
            not global_value_tweak or not global_value_tweak.hide_unavailable or
                not not managers.dlc:is_global_value_unlocked(tweak.global_value)) then
    ...
    for sort_number, player_style in ipairs(sort_data) do
        ...
        local global_value = tweak.global_value
        local dlc = tweak.dlc or global_value and managers.dlc:global_value_to_dlc(global_value)
        local achievement_locked = managers.dlc:is_content_achievement_locked("player_style", player_style) or
            managers.dlc:is_content_achievement_milestone_locked("player_style", player_style)
    ...
    local mannequin_player_style = data.mannequin_player_style or
        managers.menu_scene and managers.menu_scene:get_player_style() or "none"
    ...
    for i = 1, max_items do
        ...
        if player_style then
            ...
            local is_dlc_locked = tweak_data.lootdrop.global_values[new_data.global_value] and
                tweak_data.lootdrop.global_values[new_data.global_value].dlc and
                not managers.dlc:is_dlc_unlocked(new_data.global_value)
...
function BlackMarketGui:populate_suit_variations(data)
    ...
    for i = 1, max_items do
        ...
        if suit_variation then
            ...
            guis_catalog = "guis/"
            bundle_folder = suit_variation_data and suit_variation_data.texture_bundle_folder or
                player_style_data.texture_bundle_folder
            ...
            new_data.name_localized = suit_variation_data and managers.localization:text(suit_variation_data.name_id) or
                managers.localization:text("menu_default")
...
function BlackMarketGui:choose_weapon_mods_callback(data)
    ...
    if not _G.ch_settings.settings.u24_progress then
        if table.size(all_cosmetics) > 0 then
            ...
            for sort_number, weapon_color in ipairs(all_cosmetics_sorted) do
                ...
                local dlc_unlocked = not dlc or managers.dlc:is_dlc_unlocked(dlc)
                local unlocked = dlc_unlocked
                local achievement_locked = managers.dlc:is_content_achievement_locked("weapon_skin", weapon_color.id) or
                    managers.dlc:is_content_achievement_milestone_locked("weapon_skin", weapon_color.id)
    ...
    for i, id in ipairs(sort_mods) do
        do
            ...
            if id ~= (_G.ch_settings.settings.u24_progress and "charm" or "") then
                table.insert(new_node_data, {
                    name = id,
                    category = data.category,
                    prev_node_data = data,
...
function BlackMarketGui:sell_stashed_mask_callback(data)
    ...
    params.money = managers.experience:cash_string(managers.money:get_mask_sell_value(data.name, data.global_value,
        blueprint))
...
function BlackMarketGui:remove_mask_callback(data)
    ...
    for category, part in pairs(blueprint) do
        local converted_category = category == "color" and "colors" or category == "material" and "materials" or
            category == "pattern" and "textures" or category
        local default = managers.blackmarket:customize_mask_category_default(converted_category, true) or {}
        if default.id ~= part.id and part.id ~= "no_color_no_material" and
            managers.money:get_mask_part_price(converted_category, part.id, part.global_value) == 0 then
...
function BlackMarketGui:choose_mask_buy_callback(data)
    ...
    for _, global_value in ipairs(global_value_sort) do
        ...
        local name_id = tweak_data.lootdrop.global_value_category[global_value] and
            tweak_data.lootdrop.global_value_category[global_value].name_id or
            tweak_data.lootdrop.global_values[global_value].name_id
        if not _G.ch_settings.settings.u24_progress then
            table.insert(new_node_data, {
                name = global_value,
                category = data.category,
            ...
            if global_value ~= "mod" and global_value ~= "global_event" and global_value ~= "collaboration" then
                table.insert(new_node_data, {
                    name = global_value,
                    category = data.category,
                    prev_node_data = data,
...
if _G.ch_settings.settings.u24_progress then
    function BlackMarketGui:populate_buy_mask(data)
        ...
        local whitelisted_masks = {}

        for i = 1, max_masks do
            if table.contains(masks, mask_list[i].mask_id) then
                table.insert(whitelisted_masks, mask_list[i])
            end
        end

        max_masks = #whitelisted_masks

        for i = 1, max_masks do
            local guis_mask_id = whitelisted_masks[i].mask_id
            if tweak_data.blackmarket.masks[guis_mask_id].guis_id then
                guis_mask_id = tweak_data.blackmarket.masks[guis_mask_id].guis_id
            end
            ...
            new_data = {
                name = whitelisted_masks[i].mask_id
            }
            new_data.name_localized = managers.localization:text(tweak_data.blackmarket.masks[new_data.name].name_id)
            new_data.category = data.category
            new_data.slot = data.prev_node_data and data.prev_node_data.slot
            new_data.global_value = whitelisted_masks[i].global_value
            new_data.global_value_category = data.name
            new_data.unlocked = managers.blackmarket:get_item_amount(new_data.global_value, "masks", new_data.name, true)
                or 0
            new_data.equipped = false
            ...
            local dlc = tweak_data.blackmarket.masks[new_data.name].dlc or
                managers.dlc:global_value_to_dlc(new_data.global_value)
            if dlc and not managers.dlc:is_dlc_unlocked(dlc) then
                new_data.unlocked = -math.abs(new_data.unlocked)
                new_data.lock_texture = self:get_lock_icon(new_data)
                local dlc_global_value = managers.dlc:dlc_to_global_value(dlc)
                new_data.dlc_locked = dlc_global_value and tweak_data.lootdrop.global_values[dlc_global_value] and
                    tweak_data.lootdrop.global_values[dlc_global_value].unlock_id or "bm_menu_dlc_locked"
            elseif managers.dlc:is_content_achievement_locked(data.category, new_data.name) or
                managers.dlc:is_content_achievement_milestone_locked(data.category, new_data.name) then
                new_data.unlocked = -math.abs(new_data.unlocked)
                new_data.lock_texture = "guis/textures/pd2/lock_achievement"
            elseif managers.dlc:is_content_skirmish_locked(data.category, new_data.name) and
                (not new_data.unlocked or new_data.unlocked == 0) then
                new_data.lock_texture = "guis/textures/pd2/skilltree/padlock"
            elseif managers.dlc:is_content_crimespree_locked(data.category, new_data.name) and
                (not new_data.unlocked or new_data.unlocked == 0) then
                new_data.lock_texture = "guis/textures/pd2/skilltree/padlock"
            elseif managers.dlc:is_content_infamy_locked(data.category, new_data.name) and
                (not new_data.unlocked or new_data.unlocked == 0) then
    ...
    function BlackMarketGui:mask_mods_callback(data)
        ...
        for _, category in pairs(list) do
            ...
            if category == "materials" or category == "textures" then
                for i = 1, #items do
                    local part_id = items[i].mask_id or items[i].id
                    local part = tweak_data.blackmarket[category][part_id]
                    local allowed_dlc = {
                        "armored_transport",
                        "preorder",
                        "gage_pack",
                    ...
                    if not part.dlc or table.contains(allowed_dlc, part.dlc) then
                        if part.global_value ~= "halloween" then
                            table.insert(whitelisted_mods, items[i])
                        end
                    end
                end
            else
                whitelisted_mods = items
            end

            for i = 1, #whitelisted_mods do
                td = tweak_data.blackmarket[category][whitelisted_mods[i].id]

                if not listed_items[whitelisted_mods[i].id] and td.texture or td.colors or td.color then
                    table.insert(mods, whitelisted_mods[i])
...
function BlackMarketGui:buy_mod_callback(data)
    ...
    params.category = data.category
    params.slot = data.slot
    params.weapon_name = managers.weapon_factory:get_weapon_name_by_factory_id(managers.blackmarket:get_crafted_category(data
        .category)[data.slot].factory_id)
    params.add = true
...
function BlackMarketGui:_buy_mod_callback(data)
    self._item_bought = true
    managers.menu_component:post_event("item_buy")
    managers.blackmarket:buy_and_modify_weapon(data.category, data.slot, data.global_value, data.name, false,
        data.free_of_charge)
...
function BlackMarketGui:preview_weapon_with_mod_callback(data)
    managers.blackmarket:view_weapon_with_mod(data.category, data.slot, data.name,
        callback(self, self, "_open_preview_node"))
end
...
function BlackMarketGui:remove_mod_callback(data)
    ...
    local replaces, removes = managers.blackmarket:get_modify_weapon_consequence(data.category, data.slot,
        data.default_mod or data.name, true)
    local weapon_id = managers.blackmarket:get_crafted_category(data.category)[data.slot].weapon_id
    local cost = managers.money:get_weapon_modify_price(weapon_id, data.name, data.global_value) or 0
    params.money = cost > 0 and managers.experience:cash_string(cost)
...
function BlackMarketGui:preview_weapon_without_mod_callback(data)
    if data.default_mod then
        managers.blackmarket:view_weapon_with_mod(data.category, data.slot, data.default_mod,
            callback(self, self, "_open_preview_node"))
    else
        managers.blackmarket:view_weapon_without_mod(data.category, data.slot, data.name,
            callback(self, self, "_open_preview_node"))
    end
end
