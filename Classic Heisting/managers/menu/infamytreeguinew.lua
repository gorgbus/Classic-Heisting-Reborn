...
function InfamyTreeGui:_setup()
	...
	if managers.experience:current_rank() == 0 then
		managers.menu:show_infamous_message(MenuCallbackHandler:can_become_infamous() and managers.money:offshore() >= Application:digest_value(tweak_data.infamy.offshore_cost[managers.experience:current_rank() + 1], false))
	end
end
...
function InfamyTreeGui:reload()
	...
	local tree_cols = tweak_data.infamy.tree_cols or 4
...
function InfamyTreeGui:_select_item(index)
	...
	if self._selected_item ~= index then
		if self._selected_item then
			...
			selected_item.panel:child("text"):set_alpha(1)
		end
		...
		item.panel:child("text"):set_alpha(1)
...
function InfamyTreeGui:_update_description(name, unlocked)
	...
	if name then
		...
		if infamy_tweak.add_tier then
			...
			if index > 0 then
				local tier = math.floor((index + 2) / 4)
