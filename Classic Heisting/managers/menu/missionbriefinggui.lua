function MissionBriefingGui:init(saferect_ws, fullrect_ws, node)
	...
	self._ready_tick_box:set_image(
		self._ready and "guis/textures/pd2/mission_briefing/gui_tickbox_ready"
			or "guis/textures/pd2/mission_briefing/gui_tickbox"
	)
	...
	local x, y =
		managers.gui_data:safe_to_full_16_9(self._ready_button:world_right(), self._ready_button:world_center_y())
	...
	self._description_item = DescriptionItem:new(
		self._panel,
		utf8.to_upper(managers.localization:text(description_text_id)),
		index,
		self._node:parameters().menu_component_data.saved_descriptions
	)
	...
	if not managers.skirmish:is_skirmish() then
		self._assets_item = AssetsItem:new(
			self._panel,
			managers.preplanning:has_current_level_preplanning()
					and managers.localization:to_upper_text("menu_preplanning")
				or utf8.to_upper(managers.localization:text("menu_assets")),
			index,
			{},
			nil,
			asset_data
	...
	if managers.crime_spree:is_active() then
		...
		self._gage_assets_item =
			GageAssetsItem:new(self._panel, managers.localization:to_upper_text("menu_cs_gage_assets"), index)
	...
	self._new_loadout_item =
		NewLoadoutTab:new(self._panel, managers.localization:to_upper_text("menu_loadout"), index, loadout_data)
	...
	if not Global.game_settings.single_player then
		self._team_loadout_item =
			TeamLoadoutItem:new(self._panel, utf8.to_upper(managers.localization:text("menu_team_loadout")), index)
	...
	if managers.mutators and managers.mutators:are_mutators_active() then
		self._mutators_item =
			MutatorsItem:new(self._panel, utf8.to_upper(managers.localization:text("menu_mutators")), index)
	...
	if music_type == "heist" then
		self._jukebox_item =
			JukeboxItem:new(self._panel, utf8.to_upper(managers.localization:text("menu_jukebox")), index)
	...
	elseif music_type == "ghost" then
		self._jukebox_item =
			JukeboxGhostItem:new(self._panel, utf8.to_upper(managers.localization:text("menu_jukebox")), index)
	...
	local mutators_active = managers.mutators:are_mutators_enabled()
		and managers.mutators:allow_mutators_in_level(managers.job:current_level_id())
...
function MissionBriefingGui:mouse_pressed(button, x, y)
	...
	if
		game_state_machine:current_state().blackscreen_started
		and game_state_machine:current_state():blackscreen_started()
	then
		return
	end
...
function MissionBriefingGui:special_btn_pressed(button)
	...
	elseif button == Idstring("menu_toggle_pp_breakdown") then
		if
			managers.preplanning:has_current_level_preplanning()
			and self._assets_item
			and self._items[self._selected_item] == self._assets_item
		then
...
function NewLoadoutTab:populate_category(data, category)
	...
	for i, crafted in pairs(crafted_category) do
		guis_catalog = "guis/"
		local bundle_folder = tweak_data.weapon[crafted.weapon_id]
			and tweak_data.weapon[crafted.weapon_id].texture_bundle_folder
		...
		new_data.global_value = tweak_data.weapon[new_data.name] and tweak_data.weapon[new_data.name].global_value
			or "normal"
		...
		new_data.name_color = crafted.locked_name
			and crafted.cosmetics
			and tweak_data.economy.rarities[tweak_data.blackmarket.weapon_skins[crafted.cosmetics.id].rarity or "common"].color
		...
		local icon_list =
			managers.menu_component:create_weapon_mod_icon_list(crafted.weapon_id, category, crafted.factory_id, i)
		local icon_index = 1
		new_data.mini_icons = {}
...
function AssetsItem:create_assets(assets_names, max_assets)
		...
		if i <= #assets_names then
			...
			if not assets_names[i][3] then
				local lock = self._panel:bitmap({
					...
					texture = assets_names[i][5] and "guis/textures/pd2/blackmarket/money_lock"
						or "guis/textures/pd2/skilltree/padlock",
	...
	if not managers.menu:is_pc_controller() then
		...
		for i, string_id in ipairs(legends) do
			...
			t_text = t_text
				.. spacing
				.. utf8.to_upper(managers.localization:text(string_id, {
...
function AssetsItem:select_asset(i, instant)
	...
	if self._asset_locked[i] then
		local can_client_unlock = managers.assets.ALLOW_CLIENTS_UNLOCK == true
			or (
				type(managers.assets.ALLOW_CLIENTS_UNLOCK) == "string"
				and managers.player:has_team_category_upgrade("player", managers.assets.ALLOW_CLIENTS_UNLOCK)
			)
		...
		if is_server and can_unlock then
			extra_string = extra_string
				.. managers.localization:text("st_menu_cost")
				.. " "
				.. managers.experience:cash_string(
					managers.money:get_mission_asset_cost_by_id(self._assets_names[i][4])
				)
				.. "\n"
			...
			extra_string = extra_string
				.. managers.localization:text(
					(not is_server) and "menu_briefing_asset_server_locked"
						or (managers.assets:get_asset_unlock_text_by_id(self._assets_names[i][4]))
				) -- "menu_briefing_unable_to_unlock" )
		end
		extra_color = extra_color
			or (can_unlock and tweak_data.screen_colors.text or tweak_data.screen_colors.important_1)
	end
...
function AssetsItem:update_asset_positions()
	...
	for i, asset in pairs(self._assets_list) do
		...
		self._panel
			:child("bg_rect_" .. tostring(i))
			:set_left(math.round(self._panel:child("bg_rect_" .. tostring(i)):left()))
...
function AssetsItem:mouse_moved(x, y)
	...
	if not self._asset_selected then
		update_select = true
	elseif
		self._assets_list[self._asset_selected]
		and not self._panel:child("bg_rect_" .. tostring(self._asset_selected)):inside(x, y)
		and self._assets_list[self._asset_selected]:visible()
	then
...
function MissionBriefingGui:show()
	...
	if self._lobby_mutators_text then
		self._lobby_mutators_text:set_visible(
			managers.mutators:are_mutators_enabled()
				and managers.mutators:allow_mutators_in_level(managers.job:current_level_id())
		)
	end
end
