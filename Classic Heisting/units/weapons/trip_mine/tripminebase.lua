function TripMineBase:_sensor(t)
	...
	if ray and ray.unit and not tweak_data.character[ray.unit:base()._tweak_table].is_escort then
		...
		if not self._sensor_units_detected[ray.unit:key()] then
			...
			if managers.player:has_category_upgrade("trip_mine", "sensor_toggle") and (managers.groupai:state():whisper_mode() and tweak_data.character[ray.unit:base()._tweak_table].silent_priority_shout or tweak_data.character[ray.unit:base()._tweak_table].priority_shout) then
